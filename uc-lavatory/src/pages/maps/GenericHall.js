 ...
 return (
   <h3 className='average'>
       {props.ratingAvg.map((val) =>{
           return <h2>Overall rating: {val['avg(revRating)']}</h2>
       })}
   </h3>
 )
}
     ...
     <div className='accordion'>
       {displayList.map((val) => (
             ...
             <div className='banner' onClick={ ()=> toggle(val)}>
               <h1 className='review title'><h2 className='star'>{val.revRating}☆</h2> {val.revGender} washroom on floor {val.revFloor}</h1>
               <span>
                 {selected === val ? '-' : '+'}
               </span>
             </div>
